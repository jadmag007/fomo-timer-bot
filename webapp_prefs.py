"""Настройки страницы таймеров: тихий режим (кто кого тревит пушами).

Страница таймеров (в обычном браузере на устройстве с ботом) показывает
все таймеры сразу и даёт кнопки управления. Одна из них — «тихий режим»:
можно выключить пуши для отдельной группы (например, клановые сундуки
каждые N часов шумят) или для всех сразу. Обычные уведомления при этом
продолжают работать —
  ...
  * у замьюченных групп пуш НЕ отправляется вовсе: таймер тихо помечается
    ...
    всё видно на самой странице, где тишина управляется одной кнопкой);
...
переживают рестарт, правятся со страницы на лету. Файл крошечный, читаем
...
def set_bucket(bucket: str, muted: bool) -> bool:
    """Заглушить/вернуть звук одной группе (кнопка колокольчика на странице)."""
    with _LOCK:
        d = _load()
...
def set_all(all_muted: bool) -> bool:
    """Глобальный тихий режим (кнопка 🔕 на странице таймеров)."""
    with _LOCK:
        d = _load()
